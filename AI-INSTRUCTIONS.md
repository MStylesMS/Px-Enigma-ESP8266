...
"enigma" code-entry prop. Players manipulate a 4 Ã— 5 matrix of toggle switches,
...
- **Displays:** 2 Ã— Adafruit-style HT16K33 4-digit 7-segment, I2C `0x70` and `0x71`
- **Inputs:** 4 Ã— 5 switch matrix (4 driven outputs, 5 sensed inputs with `INPUT_PULLUP`)
...
|---|---|
| `clock_engine` *(no â€” see code_engine)* | n/a |
...
| `log` | `pxlog::info/warn/err` macros â€” Serial + ring buffer |
...
- **Persisted config** lives in `data/config.json` (LittleFS). It is
  ...
  generates defaults at boot â€” **do not flash placeholder credentials**.
...
- **Live-mode solve events are not single-shot.** `code_solved` fires
  every time the code transitions from unmatched â†’ matched; `code_unsolved`
  fires every time matched â†’ unmatched. There is no "already solved" guard
  in live mode. Only latching mode uses a single-shot `solve` event.
- **Logging:** use `pxlog::info(tag, fmt, ...)` etc. â€” never `Serial.print*`
...
- `getState` â€” publish a fresh `state` snapshot on demand.
- `restart` â€” reboot the device.
- `identify` â€” flash all `8`s on the displays for ~2 s, then resume.
- `ping` â€” outcome event only (`pong`).

Project-specific (see `docs/functional-spec.md` for full schemas):

- `setBrightness` â€” display brightness 0â€“15 (HT16K33 native range).
- `setTarget` / `clearTarget` â€” set or clear the target code.
- `setMode` â€” switch between `live` and `latching` puzzle modes.
- `reset` â€” clear `LATCHED` state and return to `ACTIVE`.
- `getCode` â€” publish the current code immediately.
- `setBatteryProfile` â€” select a built-in profile or supply a custom curve.
- `setSignalIndicator` â€” toggle / retune the decimal-point WiFi+MQTT indicator.
- `on` / `off` â€” enable / blank the display (network surface stays up).
...
are load-bearing** â€” the hardware exists.

- I2C: `SDA = GPIO0` (D3), `SCL = GPIO2` (D4), 4.7 kÎ© pull-ups.
...
- Battery sense: `A0` with calibration `V = 0.0531 Ã— raw + 0.1978`.
- Display I2C addresses: `0x70` (low pair) and `0x71` (high pair).

GPIO0, GPIO2, GPIO15 carry boot-strap responsibilities â€” the schematic accounts
for this; firmware must not reconfigure them in conflicting ways during boot.

## Testing

- `pio test -e native` â€” host-side Unity smoke tests for pure logic
